...
export function SectionHeading({ eyebrow, title, intro, align = "left", className }: SectionHeadingProps) {
  return (
    <div className={cn("max-w-2xl", align === "center" && "mx-auto text-center", className)}>
      ...
      <h2 className="font-heading text-3xl font-semibold text-foam sm:text-4xl lg:text-5xl">{title}</h2>
      {intro && <p className="mt-4 text-base leading-relaxed text-mist sm:text-lg">{intro}</p>}
    </div>
  );
}

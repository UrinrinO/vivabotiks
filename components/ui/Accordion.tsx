...
export function Accordion({ items }: { items: AccordionEntry[] }) {
  ...
  return (
    <div className="divide-y divide-foam/10 rounded-2xl border border-foam/10 bg-navy-deep/60">
      {items.map((item, i) => {
        ...
        return (
          <div key={item.question}>
            <button
              ...
              className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left font-heading text-base font-medium text-foam hover:text-accent"
            >
              {item.question}
